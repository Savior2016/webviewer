...
处理规则：
1. 如果用户想**记录**出行、搬家、物品等信息（创建/添加操作）：
   - 选择合适的项目
   - **执行具体的记录动作**（调用相应的 API 保存数据）
   - 返回 success=true，包含 project、action、data、**refresh**（跳转到对应页面）

2. 如果用户想**查询/搜索**物品、出行等信息（查找操作）：
   - 直接在 message 字段返回查询结果
   - **不要设置 refresh 字段**（设为 null），不要跳转页面
   - 返回 success=true，project=null，action=null

3. 如果用户是问候、闲聊、提问（如"你好"、"你是谁"、"能做什么"）：
   - 正常对话回复
...
{{
  ...
  "refresh": "/页面路径/ 或 null（搜索操作必须为 null）",
  "data": {{保存的数据}} 或 null
}}"""
...
def process_via_openclaw_agent(message: str, module: str = 'siri_dream') -> dict:
    ...
    try:
        ...
        if result:
            ...
            # 后处理：搜索/查询操作不设置 refresh（避免跳转）
            action = result.get('action', '')
            if action in ['search_item', 'query_location', 'query_item']:
                result['refresh'] = None
                print(f"🔧 检测到搜索操作，清除 refresh 字段")
...
def parse_intent_directly(message: str) -> dict:
    ...
    # Momhand - 物品搜索（找、查询）
    if any(kw in msg for kw in ['找', '查询', '在哪里']):
        return {
            "success": True,
            "project": "momhand",
            "action": "search_item",
            "message": "由于 AI 处理超时，请手动访问 Momhand 页面搜索物品",
            "refresh": None,  # 搜索操作不跳转
            "data": {},
            "processed_by": "direct_parse"
        }
    
    # Momhand - 物品记录（放在、位置、记录）
    if any(kw in msg for kw in ['物品', '东西', '放在', '位置', 'momhand']):
        return {
            "success": True,
            "project": "momhand",
            "action": "add_item",
