...
class WebViewerHandler(SimpleHTTPRequestHandler):
    ...
    def handle_logout(self):
        ...
        html = '''<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>退出登录</title>
<script>
localStorage.removeItem('wv_is_admin');
localStorage.removeItem('wv_is_logged_in');
setTimeout(() => { window.location.href = '/login'; }, 500);
</script>
</head><body><p>正在退出登录...</p></body></html>'''
    ...
    <script>
        ...
        document.getElementById('loginForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (data.success) {
                    // 保存登录状态到 localStorage（因为 HttpOnly Cookie JS 读不到）
                    localStorage.setItem('wv_is_admin', 'true');
                    localStorage.setItem('wv_is_logged_in', 'true');
        ...
        setInterval(async () => {{
            try {{
                ...
                if (data.status === 'approved') {{
                    // 保存访客登录状态
                    localStorage.setItem('wv_is_admin', 'false');
                    localStorage.setItem('wv_is_logged_in', 'true');
