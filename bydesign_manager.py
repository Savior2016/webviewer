...
class ByDesignManager:
    ...
    def update_trip_checklist_item(self, trip_id, item_id, updates):
        """更新出行检查清单项"""
        with _lock:
            trip = self.get_trip(trip_id)
            if not trip:
    ...
    def update_trip_custom_item(self, trip_id, item_id, updates):
        """更新出行自定义项"""
        with _lock:
            trip = self.get_trip(trip_id)
            if not trip:
    ...
    def delete_trip_custom_item(self, trip_id, item_id):
        """删除出行自定义项"""
        with _lock:
            trip = self.get_trip(trip_id)
            if not trip:
